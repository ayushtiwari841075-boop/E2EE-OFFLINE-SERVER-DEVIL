...
@app.route('/')
def home():
    return "E2EE Automation Server is Running!"
...
def setup_browser(automation_state=None):
    ...
    try:
        ...
        try:
            driver = webdriver.Chrome(options=chrome_options)
            log_message('Chrome started with alternative method!', automation_state)
            return driver
        except Exception as e2:
            log_message(f'All browser setup methods failed: {e2}', automation_state)
            raise error
...
if __name__ == '__main__':
    ...
    if 'RENDER' in os.environ or 'PORT' in os.environ:
        ...
        # For Render, we run both Flask and Streamlit
        import multiprocessing
        
        def run_streamlit():
            # This would typically be handled by Render's startup command
